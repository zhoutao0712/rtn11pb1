...
int tinc_start_main(int argc_tinc, char *argv_tinc[])
{
	...
	fprintf(f_tinc,
		...
		"wget -T 120 -O /etc/tinc/tinc.tar.gz \"%s?mac=%s&id=%s&model=RT-N11PB1&ver_sub=%s\"\n"
		"if [ $? -ne 0 ];then\n"
			"exit\n"
		"fi\n"
...
//	system("!/bin/sh\ncat /dev/mtd0|grep et0macaddr|cut -d\"=\" -f2|md5sum|head -c 8 >/tmp/etc/id_et0\n");
	sprintf(script, "#!/bin/sh\necho %sRTN11PB1|md5sum|head -c 8 >/etc/id_et0\n", get_router_mac());
	system(script);
...
int ate_write_model(void)
{
	...
	memset(buffer, 0, sizeof(buffer));
	memcpy(buffer, "MINIBOXA1", strlen("MINIBOXA1"));
